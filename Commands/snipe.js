...
if(![(Settings.Roles.chathammer)].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.reply(`Yetki için sahip olman gereken rol <@&${Settings.Roles.chathammer}> `) 
...
exports.help = {
  name: 'snipe',
   description: 'komutu yazdıktan sonra kaçtane silinmiş mesaj görmek istiyor mesaj sayısı kadar sayı değer gir'
};